...
class JwtSecurityIntegrationTest {
	...
	void shouldAllowAccessWithValidToken() throws Exception {
		...
		mockMvc.perform(get("/me").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)).andExpect(status().isOk());
	}

	@Test
	void shouldReturnUnauthorizedWithInvalidToken() throws Exception {
		mockMvc.perform(get("/me").header(HttpHeaders.AUTHORIZATION, "Bearer invalid-token"))
